...
public class ArgumentParser {
	...
	/** Initialize an empty argument map. */
	public ArgumentParser() {
		this.map = new HashMap<>();
	}

	/**
	 * Initializes argument map and fills it with data from arguments.
	 *
	...
	public void parse(String[] args) {
		...
		for (int i = 0; i < inputArgLength; i++) {
			// add it to the map if it's either a flag or a value
			if (isFlag(args[i])) {
				map.put(args[i], null);
			}
			else if (isValue(args[i]) && i > 0) {
				if (isFlag(args[i - 1])) {
	...
	/**
	 * Sets a specified path to a specified flag.
	 * 
	 * @param flag  the flag whose value path to set
	 * @param value the path to map the flag to
	 */
	public void setPath(String flag, Path value) {
		this.map.put(flag, value.toString());
	}

	/**
	 * Retrieves the string representation of the argument map.
	 * 
	 * @return the map as a {@link String}
	 */
	@Override
